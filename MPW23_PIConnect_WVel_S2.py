...
for dev in [dev for dev in ChipUT.devices if dev["DeviceType"] == "HHI_DFB"]:
    try:
        MS = pympw.MeasurementStructure(connex, dev, ChipUT)
...
def check_contact(DFB, test_curr = 0.05, test_volt = 1.2): #mA, V
    ...
    ms = pympw.MeasurementStructure(connex, DFB, ChipUT)
    print(ms["StructName"])
    ...
    for PD in [chan_key for chan_key in ms.channels if "PD" in chan_key]:
        ...
        if not 0.005 < abs(PDcurr*1e3) < 10:
...
measlist = []
start_ind = 0
for dd, MS in enumerate(MeasStrucs[1:]):
    ...
    for (ii,laser_current) in tqdm(enumerate(laser_currents)):
    ...
    if any([("heater" in str(meas.meas_attrs["Comments"]).lower()) for meas in MS.measurements]):
        ...
        for (ii,heater_current) in enumerate(heater_currents):
            ...
            heater_voltages[ii] = pcx.measure('cs' + MS.channels[PD]['Source_Number'], 'v')
