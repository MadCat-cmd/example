...
from pyMPW import pympw, pympw_old
...
""""""""""""""""""""""""""""""""""""""""""
'''this may help:'''
StructType_to_measure = ["WG_loss", "BJ_loss", "MMI_balance", "DFB_LIV_balance", "Bend_loss"]# "SOA_gain"]#, "DFB_LIV_balance", "Bend_loss"]#"

DFBs=[]
for dev in [dev for dev in ChipUT.devices if dev["DeviceType"] == "HHI_DFB"]:
    try:
        MS = pympw_old.MeasurementStructure(connex, dev, ChipUT)
...
# ip_PIConnect =     '172.16.32.226'
ip_PIConnect =     '172.16.31.88'
...
def check_contact(DFB, test_curr = 0.05, test_volt = 1.5): #mA, V
    
    ms = pympw_old.MeasurementStructure(connex, DFB, ChipUT)
    print(ms["StructName"])
    ...
    if any([("heater" in str(meas.meas_attrs["Comments"]).lower()) for meas in MS.measurements]):
        pcx.setlevel("cs" + str(ms.channels["HEAT"]['Source_Number']), heater_currents[len(heater_currents)//2])
        volt_h = pcx.measure('cs' + str(ms.channels["HEAT"]['Source_Number']), 'v')
        print(" HEAT: V = " + str(volt_h) + " V at I = "  + str(heater_currents[len(heater_currents)//2]*1e3) + " mA")
...
measlist = []
start_ind = 15
for dd, MS in enumerate(MeasStrucs[start_ind:]):
    ...
    for meas in MS.measurements:
        ...
        # if "heater" in meas.meas_attrs["Comments"].lower():
        #     print("not uploading heater vi")
        #     continue
